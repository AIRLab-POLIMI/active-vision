...
# This launch is responsible for launching the OctoMap creation process with the color filtering 
# segmentation for the Igus ReBeL robot. This launch file includes various arguments and configurations to customize 
# the launch process, such as enabling or disabling specific components, setting the robot's initial position, and 
# selecting the camera and end effector. The launch file is structured such that is is possible to run the entire process 
# or individual elements such as the pointcloud creation or the extended octomap creation, or only the segmentation 
# without update the octomap.
...
def launch_setup(context, *args, **kwargs):
    ...
    if config_yaml['launch']['octomap_segmentation_color_filter_launch']['extended_octomap_creation_launch']['partial_pointcloud_subscription'] == "False":
    ...
    octomap_semantic_class_cells_vis_topic = config_yaml['topics']['segmentation']['color_filter']['octomap_semantic_class_cells_vis_topic']
    ...
    if LaunchConfiguration("run_robot").perform(context) == 'true': 
        if LaunchConfiguration("load_gazebo").perform(context) == 'true' and LaunchConfiguration("hardware_protocol").perform(context) == 'ignition': 
            ...
            realsense_launch_file = IncludeLaunchDescription(
                PythonLaunchDescriptionSource([
                    FindPackageShare("av_bringup"), '/launch', '/realsense.launch.py']),
                launch_arguments={
                    **use_sim_time_dict,
                }.items(),
            )
            ...
            moveit_launch_file = IncludeLaunchDescription(
                ...
                launch_arguments={
                    "rviz_file": os.path.join(
                        get_package_share_directory("av_bringup"), "rviz/", config_yaml["launch"]["octomap_segmentation_color_filter_launch"]["rviz"]["real"] ),
                }.items(),
            )
    ...
    color_filter_segmentation_launch_file = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([
            FindPackageShare("av_segmentation_color_filtering"), '/launch', '/color_filter.launch.py']),
        launch_arguments={
            **use_sim_time_dict,
            **{
    ...
    pointcloud_creation_launch_file = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([
            FindPackageShare("av_pointcloud"), '/launch', '/pointcloud_creation.launch.py']),
        launch_arguments={
            **use_sim_time_dict,
            **{
                "depth_image_topic": depth_image_topic,
    ...
    segmented_pointcloud_creation_launch_file = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([
            FindPackageShare("av_pointcloud"), '/launch', '/segmented_pointcloud_creation.launch.py']),
        launch_arguments={
            **use_sim_time_dict,
            **{
            ...
            **config_yaml['launch']['octomap_segmentation_color_filter_launch']['segmented_pointcloud_creation_launch']
        }.items(),
    ) 
    ...
    extended_octomap_creation_launch_file = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([
            FindPackageShare("av_octomap"), '/launch', '/extended_octomap_creation.launch.py']),
        launch_arguments={
            **use_sim_time_dict,
            **{
                "pointcloud_topic": pointcloud_topic,
                "partial_tf_topic": color_filter_tf_topic,
                "segmented_pointcloud_topic": segmented_pointcloud_topic,
                "segmented_pointcloud_tf_topic": color_filter_tf_topic,
            ...
            **config_yaml['launch']['octomap_segmentation_color_filter_launch']['extended_octomap_creation_launch'],
        }.items(),
    ) 
    ...
    # Rviz node
    if LaunchConfiguration("load_gazebo").perform(context) == 'true':
        rviz_config_file_name = config_yaml["launch"]["octomap_segmentation_color_filter_launch"]["rviz"]["ignition"]
    else:
        if LaunchConfiguration("test_camera").perform(context) == 'false':
            rviz_config_file_name = config_yaml["launch"]["octomap_segmentation_color_filter_launch"]["rviz"]["real"]
        else:
            if LaunchConfiguration("camera").perform(context) == 'realsense':
                rviz_config_file_name = config_yaml["launch"]["octomap_segmentation_color_filter_launch"]["rviz"]["realsense"]
    ...
    if LaunchConfiguration("run_color_filter").perform(context) == 'true': 
    ...
    if LaunchConfiguration("run_pt").perform(context) == 'true' and config_yaml['launch']['octomap_segmentation_color_filter_launch']['extended_octomap_creation_launch']['partial_pointcloud_subscription'] == "False": 
