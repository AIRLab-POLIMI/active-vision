...
from av_interfaces.srv import LANGSAMSegmentation

from av_segmentation_lang_sam.utils import (
    merge_masks_images,
    show_masks_images,
...
class LANGSAMClient(Node):
    ...
    def sync_segment_request(
        ...
        original_image = cv2.imread(os.path.join(
            get_package_share_directory("av_segmentation_lang_sam"), "data", self._image_name))
    ...
    try:
        ...
        if len(masks_images) <= 0:
            ...
            export_masks_images(masks_images, os.path.join(
                get_package_share_directory("av_segmentation_lang_sam"), "data", "masks"))
