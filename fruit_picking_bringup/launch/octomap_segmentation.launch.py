...
def launch_setup(context, *args, **kwargs):
    ...
    segmented_pointcloud_creation_launch_file = IncludeLaunchDescription(
        ...
        launch_arguments={
            ...
            **config_yaml['launch']['octomap_segmentation_launch']['segmented_pointcloud_creation_launch']
        }.items(),
    ) 
    ...
    extended_octomap_creation_launch_file = IncludeLaunchDescription(
        ...
        launch_arguments={
            ...
            **config_yaml['launch']['octomap_segmentation_launch']['extended_octomap_creation_launch'],
        }.items(),
    ) 
    ...
    # Rviz node
    if LaunchConfiguration("load_gazebo").perform(context) == 'true':
        rviz_config_file_name = config_yaml["launch"]["octomap_segmentation_launch"]["rviz"]["ignition"]
    else:
        if LaunchConfiguration("test_camera").perform(context) == 'false':
            rviz_config_file_name = config_yaml["launch"]["octomap_segmentation_launch"]["rviz"]["real"]
        else:
            if LaunchConfiguration("camera").perform(context) == 'realsense':
                rviz_config_file_name = config_yaml["launch"]["octomap_segmentation_launch"]["rviz"]["realsense"]
